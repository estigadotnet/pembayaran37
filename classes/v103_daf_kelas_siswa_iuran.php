...
class v103_daf_kelas_siswa_iuran extends DbTable
{
	...
	public function __construct()
	{
		...
		// siswa_id
		$this->siswa_id = new DbField('v103_daf_kelas_siswa_iuran', 'v103_daf_kelas_siswa_iuran', 'x_siswa_id', 'siswa_id', '`siswa_id`', '`siswa_id`', 3, 11, -1, FALSE, '`siswa_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// NomorInduk
		$this->NomorInduk = new DbField('v103_daf_kelas_siswa_iuran', 'v103_daf_kelas_siswa_iuran', 'x_NomorInduk', 'NomorInduk', '`NomorInduk`', '`NomorInduk`', 200, 25, -1, FALSE, '`NomorInduk`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->NomorInduk->Sortable = TRUE; // Allow sort
		$this->fields['NomorInduk'] = &$this->NomorInduk;

		// Nama
		$this->Nama = new DbField('v103_daf_kelas_siswa_iuran', 'v103_daf_kelas_siswa_iuran', 'x_Nama', 'Nama', '`Nama`', '`Nama`', 200, 50, -1, FALSE, '`Nama`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getRecordFilter($row = NULL)
	{
		...
		if (is_array($row))
			$val = array_key_exists('siswa_id', $row) ? $row['siswa_id'] : NULL;
		else
			$val = $this->siswa_id->OldValue !== NULL ? $this->siswa_id->OldValue : $this->siswa_id->CurrentValue;
		if (!is_numeric($val))
	...
	public function getViewUrl($parm = "")
	{
		if ($parm != "")
			...
			$url = $this->keyUrl("v103_daf_kelas_siswa_iuranview.php", $this->getUrlParm(Config("TABLE_SHOW_DETAIL") . "="));
