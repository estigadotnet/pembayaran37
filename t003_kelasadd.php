		...
		for (var i = startcnt; i <= rowcnt; i++) {
			...
			<?php if ($t003_kelas_add->Nama->Required) { ?>
				...
				if (elm && !ew.isHidden(elm) && !ew.hasValue(elm))
					return this.onError(elm, "<?php echo JsEncode(str_replace("%s", $t003_kelas_add->Nama->caption(), $t003_kelas_add->Nama->RequiredErrorMessage)) ?>");
			<?php } ?>
...
<?php if ($t003_kelas_add->Nama->Visible) { // Nama ?>
	<div id="r_Nama" class="form-group row">
		<label id="elh_t003_kelas_Nama" for="x_Nama" class="<?php echo $t003_kelas_add->LeftColumnClass ?>"><?php echo $t003_kelas_add->Nama->caption() ?><?php echo $t003_kelas_add->Nama->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $t003_kelas_add->RightColumnClass ?>"><div <?php echo $t003_kelas_add->Nama->cellAttributes() ?>>
<span id="el_t003_kelas_Nama">
<input type="text" data-table="t003_kelas" data-field="x_Nama" name="x_Nama" id="x_Nama" size="30" maxlength="50" placeholder="<?php echo HtmlEncode($t003_kelas_add->Nama->getPlaceHolder()) ?>" value="<?php echo $t003_kelas_add->Nama->EditValue ?>"<?php echo $t003_kelas_add->Nama->editAttributes() ?>>
</span>
<?php echo $t003_kelas_add->Nama->CustomMsg ?></div></div>
	</div>
<?php } ?>
</div><!-- /page* -->
